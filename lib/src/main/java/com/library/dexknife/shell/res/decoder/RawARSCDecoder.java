...
public class RawARSCDecoder {
    ...
    private com.library.dexknife.shell.res.util.ExtDataInput mIn;
    private Header       mHeader;
    private StringBlock  mTypeNames;
    private StringBlock  mSpecNames;
    private com.library.dexknife.shell.res.data.ResPackage mPkg;
    private com.library.dexknife.shell.res.data.ResType mType;
    private int mCurTypeID = -1;
    private com.library.dexknife.shell.res.data.ResPackage[] mPkgs;
    private int          mResId;


    private RawARSCDecoder(InputStream arscStream) throws AndrolibException, IOException {
        mIn = new com.library.dexknife.shell.res.util.ExtDataInput(new LEDataInputStream(arscStream));
        mExistTypeNames = new HashMap<>();
    }

    public static com.library.dexknife.shell.res.data.ResPackage[] decode(InputStream arscStream
    )
        ...
        try {
            ...
            com.library.dexknife.shell.res.data.ResPackage[] pkgs = decoder.readTable();
    ...
    private com.library.dexknife.shell.res.data.ResPackage[] readTable() throws IOException, AndrolibException {
        nextChunkCheckType(Header.TYPE_TABLE);
        int packageCount = mIn.readInt();
        StringBlock.read(mIn);
        com.library.dexknife.shell.res.data.ResPackage[] packages = new com.library.dexknife.shell.res.data.ResPackage[packageCount];
        nextChunk();
    ...
    private com.library.dexknife.shell.res.data.ResPackage readPackage() throws IOException, AndrolibException {
        ...
        mResId = id << 24;
        mPkg = new com.library.dexknife.shell.res.data.ResPackage(id, name);
        nextChunk();
    ...
    private void readTableTypeSpec() throws AndrolibException, IOException {
        ...
        mType = new com.library.dexknife.shell.res.data.ResType(mTypeNames.getString(id - 1), mPkg);
        while (nextChunk().type == Header.TYPE_TYPE) {
            readConfig();
        }
    }
    ...
    public static class Header {
        ...
        public static Header read(com.library.dexknife.shell.res.util.ExtDataInput in) throws IOException {
            short type;
            try {
                type = in.readShort();
